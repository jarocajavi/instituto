<x-layouts.layout>
    <div class="max-w-2xl mx-auto">
        ...
        <div class="bg-zinc-800 rounded-xl border border-zinc-700 p-6">
            ...
            <form action="{{ route('crud.store', $resource) }}" method="POST">
                @csrf
                            ...
                            @if($type === 'textarea')
                                <textarea name="{{ $field }}" rows="3"
                                    class="w-full bg-zinc-900 border border-zinc-600 rounded-lg px-3 py-2
                                           text-zinc-200 focus:border-orange-500 focus:outline-none text-sm">{{ old($field) }}</textarea>
